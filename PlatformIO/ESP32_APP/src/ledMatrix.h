...
void drawText(int colorWheelOffset)
{
  ...
  matrix->setCursor(5, 15);
  matrix->print(str);

  matrix->setCursor(5, 25);
  matrix->print(str);

  matrix->setCursor(5, 35);
  matrix->print(str);

  matrix->setCursor(5, 45);
  matrix->print(str);

  matrix->setCursor(5, 55);
  matrix->print(str);
  /*
  for (size_t i = 1; i < 3; )
  {
    matrix->setCursor(5, 5*i); // start at top left, with 5,5 pixel of spacing
    uint8_t w = 0;
